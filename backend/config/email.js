/**
 * Email Configuration - Supports Resend API (recommended for cloud hosting)
 * and Brevo SMTP (for local development)
 *
 * Set EMAIL_PROVIDER=resend for Resend API (works on Render, Vercel, etc.)
 * Set EMAIL_PROVIDER=brevo for Brevo SMTP (works locally, may fail on cloud)
 */
...
// Brevo SMTP credentials from environment variables (fallback)
const brevoUser = process.env.EMAIL_USER;
const brevoPass = process.env.EMAIL_PASS;

let transporter = null;
if (emailProvider === 'brevo' && brevoUser && brevoPass) {
...
const getFromEmail = () => {
  ...
  if (emailProvider === 'resend') {
    return 'Starvation to Salvation <onboarding@resend.dev>';
  }
  // For Brevo
  return `Starvation to Salvation <${brevoUser}>`;
};
...
const sendEmail = async (mailOptions) => {
  try {
    ...
    console.log(`📧 Attempting to send email via ${emailProvider} to: ${mailOptions.to}`);
    ...
    if (emailProvider === 'resend' && resend) {
      ...
      console.log(`✅ Email sent via Resend: ${mailOptions.subject} to ${mailOptions.to}`);
    ...
    if (transporter) {
      ...
      console.log(`✅ Email sent via Brevo: ${mailOptions.subject} to ${mailOptions.to}`);
      return { success: true, messageId: info.messageId };
    }
    
    throw new Error('No email provider configured. Set EMAIL_PROVIDER and credentials.');
  } catch (error) {
...
export const sendEmailVerificationCode = async (email, name, code) => {
  const mailOptions = {
    to: email,
    subject: '🔐 Verify Your Email - Starvation to Salvation',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background: #f0fdf4; padding: 20px; border-radius: 10px;">
          <p>Dear ${name},</p>
          <p>Thank you for registering with Starvation to Salvation. Please use the following code to verify your email address:</p>
          ...
          <p style="color: #666;">If you didn't create an account with Starvation to Salvation, please ignore this email.</p>
        </div>
      </div>
    `,
  };
...
export const sendPasswordResetCode = async (email, name, code) => {
  const mailOptions = {
    to: email,
    subject: '🔑 Password Reset Code - Starvation to Salvation',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
...
export const sendVerificationResult = async (receiver, approved) => {
  ...
  const mailOptions = {
    to: receiver.email,
    subject: approved ? '✅ Account Verified - Starvation to Salvation' : '❌ Account Verification Update',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
...
export const sendAdminMessageToUser = async (user, subject, message, actionRequired = null) => {
  ...
  const mailOptions = {
    to: user.email,
    subject: `📬 ${subject} - Starvation to Salvation`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #3b82f6;">Message from Starvation to Salvation Admin</h2>
        <div style="background: #eff6ff; padding: 20px; border-radius: 10px;">
          <p>Dear ${user.name},</p>
        ...
        <p style="color: #999; font-size: 12px; text-align: center; margin-top: 20px;">
          This is an official message from Starvation to Salvation administration.
        </p>
      </div>
    `,
  };
